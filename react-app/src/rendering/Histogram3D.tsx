...
import Inter from '../assets/Inter_Regular_Reduced.json';
    ...
    const grid = useMemo(() => {
        ...
        for (let i = 0; i < rows; i++) {
            for (let j = 0; j < cols; j++) {
                ...
                const key = (i + 1) + "x" + (j + 1);
                if (!currentHeights.current.has(key)) {
                    currentHeights.current.set(key, defaultHeight);
                }
